...
import com.qinzhenning.BookLendSystem.GUI.AdminFrame.AddAdminDialog.button1_ActionListener;
import com.qinzhenning.BookLendSystem.GUI.AdminFrame.AddAdminDialog.button2_ActionListener;

/**
 * @author qinzhenning
 * 时间：2012-6-28下午1:41:38
 * 文件：ChangeAdminDialog.java
 * 项目：BookLendSystem 
 ...
 * 修改管理员信息 对话框
 */
public class ChangeAdminDialog extends JDialog {
	...
	private Administrator changeAdmin;
	private ArrayList<Administrator> list =  AdministratorList.getAdminList();
	
	/**
	  * 构造方法-设置标题 和 传入管理员对象
	  */
	public ChangeAdminDialog(String name,Administrator admin){
		setTitle(name);
		changeAdmin = admin;
	}
	...
	public void go(){
	   ...
	   field_username = new JTextField(changeAdmin.getUsername());		//用户名
	   gridx=1;
  	    gridy=0;
  	    gridwidth=3;
  	    gridheight=1;
  	    ...
  	    field_password = new JTextField(changeAdmin.getPassword());		//密码
  	    gridx=1;
	    gridy=1; 
	    c=new GridBagConstraints(gridx,gridy,gridwidth,gridheight,weightx,weighty,anchor,
	   	       fill,inset,ipadx,ipady);
	   ...
	   field_name = new JTextField(changeAdmin.getName());			//姓名
 	   gridx=1;
	   gridy=2; 
	   c=new GridBagConstraints(gridx,gridy,gridwidth,gridheight,weightx,weighty,anchor,
	   	       fill,inset,ipadx,ipady);
	  ...
	  if(changeAdmin.getSex().equals("男"))
	   ...
	   field_adNumber = new JTextField(changeAdmin.getAdminNumber());
	   gridx=1;
	   gridy=4; 
	   gridwidth=3;
 	   gridheight=1;
	...
	public class button1_ActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			{
				if(changeAdmin.getAdminNumber().equals(field_adNumber.getText()) 
						|| changeAdmin.getUsername().equals(field_username.getText()))
					isChange = true;
				...
				if(isChange)
				{
					...
					AdministratorList.changeAdmin(changeAdmin, field_username.getText(), field_password.getText(), 
							field_name.getText(), sex, field_adNumber.getText());
					dispose();		
					JOptionPane.showMessageDialog(null, "修改信息成功！修改结果如下");
					ManageAdmin.getTabel().setModel(new AdminTableModel(changeAdmin));
				}
				else
				{
